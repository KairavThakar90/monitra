...
import { buildDistribution, describeCoverage } from "../dashboard/v2/distribution";
import {
...
export const MemberReports: React.FC = () => {
  ...
  // own scope-wide total, never the summary strip's. See distribution.ts.
  const distributionSeconds = listData?.total_seconds ?? 0;
  const isUsageDimension = reportId === "apps" || reportId === "urls";
  const coverage = describeCoverage(distributionSeconds, totalSeconds);

  const donutSlices = useMemo(
    () =>
      buildDistribution({
        rows,
        totalSeconds: listData?.total_seconds ?? 0,
        rowCount: listData?.total ?? rows.length,
        dimensionLabel: (config?.dimensionLabel ?? "row").toLowerCase(),
        colors: DONUT_COLORS,
      }),
