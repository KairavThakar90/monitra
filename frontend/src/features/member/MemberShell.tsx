 ...
 *
 * Deliberately a sibling of `V2Shell` rather than a mode of it: the admin
 * sidebar is a map of the whole organization, and this one is a map of one
 * person's own work. Keeping them apart means a link added for admins cannot
 * appear on a member's screen by accident — the member navigation is the list
 * below and nothing else. Notably there is no Members entry: a member has no
 * business managing the organization's roster, and the backend refuses
 * `view_employees` for them anyway.
 */
...
/** The four detail reports, mirrored in the sidebar and the report switcher. */
export const MEMBER_REPORT_LINKS = [
...
const NAV: NavItem[] = [
  {
    ...
    icon: icon(
      ...
      "M20.488 9A9.004 9.004 0 0015 3.512V9h5.488z"
    ),
  },
  {
...
export const MemberShell: React.FC<{
  title: string;
  subtitle?: string;
  ...
  const onReports = location.pathname.startsWith("/member/reports");
  ...
  // A section is active for its own page and anything nested under it, so
  // drilling into /member/team/12 keeps My Team highlighted.
              ...
              {reportsOpen && (
                <ul className="ml-6 mt-1 space-y-0.5 border-l border-slate-800 pl-2.5">
                  {MEMBER_REPORT_LINKS.map((report) => {
                    const active = location.pathname === `/member/reports/${report.id}`;
                    return (
                      <li key={report.id}>
                        <button
